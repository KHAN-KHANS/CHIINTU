...
def display_tokens_table(result):
    """Display tokens in a beautiful table format"""
    
    print(GREEN + BOLD + "\n" + "╔" + "═" * 80 + "╗")
    print("║" + " " * 30 + "🔑 GENERATED TOKENS 🔑" + " " * 30 + "║")
    print("╠" + "═" * 80 + "╣")
    ...
    print(f"║ {GREEN}{result['original_token']['access_token']}{RESET}" + " " * (79 - len(result['original_token']['access_token'])) + "║")
    print("╠" + "═" * 80 + "╣")
    ...
    if 'converted_tokens' in result and result['converted_tokens']:
        ...
        for i, (app_key, token_data) in enumerate(result['converted_tokens'].items(), 1):
            if i > 5:  # Show first 5 tokens fully
                print(f"║ {YELLOW}+ {len(result['converted_tokens']) - 5} MORE TOKENS...{RESET}" + " " * 62 + "║")
                break
            ...
            print(f"║ {CYAN}[{i}] {app_key}{RESET}" + " " * (76 - len(app_key)) + "║")
            print(f"║     📱 {app_name[:30]}{RESET}" + " " * (76 - len(app_name[:30])) + "║")
            print(f"║     🔑 PREFIX: {prefix}{RESET}" + " " * (69 - len(prefix)) + "║")
            print(f"║     {GREEN}{display_token}{RESET}" + " " * (79 - len(display_token)) + "║")
    ...
    if len(cookies_string) > 70:
        cookies_display = cookies_string[:70] + "..."
    else:
        cookies_display = cookies_string
    
    print(f"║ {YELLOW}{cookies_display}{RESET}" + " " * (79 - len(cookies_display)) + "║")
    print("╚" + "═" * 80 + "╝" + RESET)
    
    print(f"\n{CYAN}📊 TOTAL TOKENS GENERATED: {len(result['converted_tokens']) + 1}{RESET}")
...
if __name__ == "__main__":
    ...
    print(f"║  {YELLOW}[2]{RESET} {GREEN}COOKIES → ALL TOKENS{RESET}" + " " * 28 + "║")
    print("╚" + "═" * 60 + "╝" + RESET)
    ...
    if option == '1':
        ...
        # OPTION 2: COOKIES TO ALL TOKENS
        print(YELLOW + BOLD + "\n╔" + "═" * 62 + "╗")
        ...
        if not token_result['success']:
            ...
            print("╚" + "═" * 62 + "╝" + RESET)
            print(f"{RED}Error: {token_result.get('error')}{RESET}")
            exit()
        ...
        # Display ALL tokens
        display_tokens_table(full_result)
        
        print(f"\n{CYAN}🎉 SUCCESSFULLY GENERATED {token_result['total_tokens']} TOKENS!{RESET}")
